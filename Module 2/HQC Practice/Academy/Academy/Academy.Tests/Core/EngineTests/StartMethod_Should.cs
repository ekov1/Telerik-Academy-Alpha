...
namespace Academy.Tests.Core.EngineTests
{
    ...
    public class StartMethod_Should
    {
        ...
        [TestMethod]
        public void CallWriterMethod_WhenTerminatorCommandIsCalled()
        {
            // Arrange
            var readerMock = new Mock<IReader>();
            var writerMock = new Mock<IWriter>();
            ...
            var databaseMock = new Mock<IDatabase>();
            var terminatorCommand = "Exit";
            var expectedDatabaseResult = new List<string> { "kuche" };

            var engine = new Engine(readerMock.Object, writerMock.Object, processorMock.Object, databaseMock.Object);

            readerMock.Setup(m => m.ReadLine()).Returns(terminatorCommand);
            databaseMock.SetupGet(m => m.ExecutionResult).Returns(expectedDatabaseResult);

            //Act
            engine.Start();

            //Assert
            writerMock.Verify(w => w.Write(string.Join("\n", expectedDatabaseResult)), Times.Once);
        }
        
        [TestMethod]
        public void ParseCommandAndAddToDatabase_WhenInputIsValidAndNotTerminating()
        {
            // Arrange
            var readerMock = new Mock<IReader>();
            var writerMock = new Mock<IWriter>();
            ...
            var databaseMock = new Mock<IDatabase>();
            var terminatorCommand = "Exit";
            var testCommand = "Simple Test Command";
            var someResult = "Some result";
            var databaseList = new List<string>();
            ...
            readerMock.SetupSequence(m => m.ReadLine())
                .Returns(testCommand)
                .Returns(terminatorCommand);

            processorMock.Setup(m => m.ProcessCommand(testCommand)).Returns(someResult);

            databaseMock.SetupGet(m => m.ExecutionResult).Returns(databaseList);

            // Act
            engine.Start();

            // Assert
            Assert.AreEqual(someResult, databaseMock.Object.ExecutionResult.Single());
        }
    }
}
